...
### Added
- Responsive mobile layout: on screens ≤640px the roles table collapses into
  stacked cards (one card per role, with labeled rows), the toolbar and header
  wrap cleanly, and the Add/Edit and detail modals switch to a single-column,
  near-full-width layout. Row actions (edit/del) are always visible on touch.
